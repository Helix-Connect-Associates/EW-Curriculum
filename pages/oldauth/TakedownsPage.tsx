...
import { getTakedowns, updateTakedown, addTakedown, deleteTakedown } from '../services/curriculumService';
...
const TakedownEditor: React.FC<{
    takedown: Omit<Takedown, 'id'> | Takedown;
    onSave: (takedown: Omit<Takedown, 'id'> | Takedown) => void;
    onCancel: () => void;
    onDelete?: () => void;
}> = ({ takedown, onSave, onCancel, onDelete }) => {
    const [edited, setEdited] = useState(takedown);

    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        setEdited({ ...edited, [e.target.name]: e.target.value });
    };

    const isNew = !('id' in takedown);

    return (
        <div className="bg-white rounded-lg border-2 border-ew-gold-dark shadow-lg p-6 my-4">
            <h3 className="text-xl font-bold font-heading mb-4">{isNew ? 'Add New Takedown' : `Editing: #${(takedown as Takedown).id}`}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" name="child_belt" value={edited.child_belt} onChange={handleChange} placeholder="Child Belt" className="p-2 border rounded" />
                <input type="text" name="adult_belt" value={edited.adult_belt} onChange={handleChange} placeholder="Adult Belt" className="p-2 border rounded" />
...
const TakedownCard: React.FC<{ takedown: Takedown; onSave: () => void; onRequestAuth: (callback: () => void) => void }> = ({ takedown, onSave, onRequestAuth }) => {
    const [isEditing, setIsEditing] = useState(false);
    const { isAuthenticated } = useAuth();

    const handleSave = (updated: Takedown) => {
        updateTakedown(updated.id, updated);
        setIsEditing(false);
        onSave();
    };
    
    const handleDelete = () => {
        if (window.confirm(`Are you sure you want to delete takedown #${takedown.id}?`)) {
            deleteTakedown(takedown.id);
            setIsEditing(false);
            onSave();
        }
    };
    ...
    if (isEditing) {
        return <TakedownEditor takedown={takedown} onSave={handleSave} onCancel={() => setIsEditing(false)} onDelete={handleDeleteClick} />;
    }
...
const TakedownsPage: React.FC = () => {
    ...
    const handleAddNew = (newTakedown: Omit<Takedown, 'id'>) => {
        addTakedown(newTakedown as Takedown);
        refreshData();
        setIsAdding(false);
    };
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
            ...
            <div className="space-y-4">
                {takedowns.map(t => <TakedownCard key={t.id} takedown={t} onSave={refreshData} onRequestAuth={handleRequestAuth} />)}
            </div>
            
            <div className="mt-12 text-center">
                {isAdding ? (
                     <TakedownEditor
                        takedown={{ child_belt: 'White', adult_belt: 'White', technique_right: '', technique_left: '', video_url: '', video_time: '' }}
                        onSave={handleAddNew}
